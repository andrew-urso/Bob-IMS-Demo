# Tenet 3 — Update Existing IMS Databases
...
**Status:** Active. Database *creation* is handled by Tenet 2.
...
1. **Always** reference DDL patterns in `context_and_examples/DDL-examples/` before generating.
...
6. Save the generated DDL to `context_and_examples/ddl-output/<DATABASE_NAME>_alter_<timestamp>.sql` **before** execution.
...
```
context_and_examples/ddl-output/<DATABASE_NAME>_alter_<timestamp>.sql
```
...
mvn exec:java -Dexec.mainClass="com.ibm.ims.jdbcbob.IMSDBAExecutor" \
  -Dexec.args="ALTER ../context_and_examples/ddl-output/<DATABASE_NAME>_alter_<timestamp>.sql"
```

> **Note:** `IMSDBAExecutor` requires the operation keyword (`ALTER`, `CREATE`, or `DROP`) as the **first** argument, followed by the DDL file path. Omitting it causes the executor to print usage and exit without executing.
...
## Bob's Full Decision Tree for Tenet 3

```
User request triggers Tenet 3 (update/alter/modify keywords)
        ↓
...
Save draft to context_and_examples/ddl-output/<DATABASE_NAME>_alter_<timestamp>.sql
        ↓
...
| `IMSDBAExecutor` exits with non-zero code | JDBC connection or DDL syntax error | Check `context_and_examples/ddl-results/` log; verify connection properties in `IMSDDL.java` |
